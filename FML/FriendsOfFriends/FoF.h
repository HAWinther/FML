...
namespace FML {
  namespace FOF {
    ...
    template<class T, int NDIM>
      void FriendsOfFriendsLinkingLocal(
          T *part, 
          size_t NumPart, 
          std::vector<FoFCells> &PartCells,
          int Ngrid,
        ...
        public:
          ...
          void add(T & particle, bool periodic){
            ...
            for(int idim = 0; idim < NDIM; idim++){
              ...
              if(periodic){
                if(dx[idim]  < -0.5) dx[idim] += 1;
                if(dx[idim] >=  0.5) dx[idim] -= 1;
              }
              pos[idim] += dx[idim] * _mass / (mass + _mass);
              if(periodic){
                if(pos[idim]  < 0.0) pos[idim] += 1;
                if(pos[idim] >= 1.0) pos[idim] -= 1;
              }
    ...
    template<class T, int NDIM>
      void ParticlesToCells(
        ...
        if(Ngrid % FML::NTasks > Ngrid/2) Ngrid *= 2;
    ...
    template<class T, int NDIM>
      void BoundaryLinking(
        ...
        for(size_t i = 0; i < NumPart; i++){
          ...
          if(FML::xmax_domain - pos[0] < fof_distance) nboundary_right++;
        }
        ...
        MPI_Sendrecv(
            &bytes_to_send, sizeof(bytes_to_send), MPI_BYTE, RightTask, 0, 
            &bytes_to_recv, sizeof(bytes_to_recv), MPI_BYTE, LeftTask,  0, 
        ...
        for(int ss = 0; ss < nmerging_steps; ss++){
          ...
          if(FML::ThisTask == sendTask || FML::ThisTask == recvTask or merging_in_parallel){
            ...
            if(FML::ThisTask == sendTask or merging_in_parallel){
              std::cout << "Sending boundary particles from " << FML::ThisTask << " to " << RightTask << " and merging FoF groups" << std::endl;
              std::cout << FML::ThisTask << " will send "    << bytes_to_send/1e3 << " kb of data to task "   << RightTask << std::endl;
            }
            if(FML::ThisTask == RightTask or merging_in_parallel){
              std::cout << FML::ThisTask << " will recieve " << bytes_to_recv/1e3 << " kb of data from task " << LeftTask  << std::endl;
            }
#endif
            ...
            if(merging_in_parallel){
              MPI_Sendrecv(
                  CommBufferSend.data(), bytes_to_send, MPI_BYTE, RightTask, 0,
                  CommBufferRecv.data(), bytes_to_recv, MPI_BYTE, LeftTask,  0, 
                  MPI_COMM_WORLD, &status);
            } else {
              ...
              if(FML::ThisTask == recvTask){
                MPI_Recv(CommBufferRecv.data(), bytes_to_recv, MPI_BYTE, LeftTask,  0, MPI_COMM_WORLD, &status);
              }
            }
            ...
            auto get_fof_id = [&](int i) -> size_t* {
              assert(FML::ThisTask == recvTask or merging_in_parallel);
              assert(size_t(i) >= 0 and size_t(i) < nboundary_left_recv);
              return (size_t *) &CommBufferRecv[bytes_per_partice * i + sizeof(FoFPosType)*NDIM + sizeof(size_t)];
            };
            ...
            if(FML::ThisTask == recvTask or merging_in_parallel){
              for(size_t i = 0; i < nboundary_left_recv; i++){
                ...
                for(int nbcell = 0; nbcell < threetondim; nbcell++){
                  ...
                  if(icoord[0] != 0) continue;

                  // Compute cell-index of nbor cell
                  size_t index_nbor_cell = 0;
                  if(!periodic) skip = false;
                  for(int idim = 0; idim < NDIM; idim++){
                    // Periodic boundary conditions
                    if(periodic){
                      if(icoord[idim] < 0) icoord[idim] += Ngrid;
                      if(icoord[idim] >= Ngrid) icoord[idim] -= Ngrid;
                    } else {
                      if(icoord[idim] < 0) skip = true;
                      if(icoord[idim] >= Ngrid) skip = true;
                    }
                    index_nbor_cell = index_nbor_cell * Ngrid + icoord[idim];
                  }
                  if(!periodic and skip) continue;
                  ...
                  for(size_t ii = 0; ii < np; ii++){
                    ...
                    if(dist2 < fof_distance2){
                      ...
                      if(original_fof_id1 == no_FoF_ID and original_fof_id2 == no_FoF_ID){
                        ...
                        // (we currently ignore this case as its only relevant if we want 2 particle FoF groups
                        // the way to deal with it is to have some free indexes for each task to be able to 
                        // assign then assign it here and increase counter)
                        continue;
                      }
                      ...
                      if(original_fof_id1 == original_fof_id2) continue;
                      if(original_fof_id1 == no_FoF_ID) {
                        // The particle has no FoF group, but belong to the FoF group of the boundary particle
...
#pragma omp parallel for
                        ...
                        for(size_t jj = 0; jj < NumPart; jj++)
                          if(particle_id_FoF[jj] == original_fof_id1 || particle_id_FoF[jj] == original_fof_id2){
            ...
            if(merging_in_parallel){
              MPI_Sendrecv(
                  &count,      sizeof(count),      MPI_BYTE, LeftTask,  0,
                  &count_recv, sizeof(count_recv), MPI_BYTE, RightTask, 0, 
                  MPI_COMM_WORLD, &status);
            } else {
              if(FML::ThisTask == recvTask)
...
#ifdef DEBUG_FOF
            if(merging_in_parallel){
              std::cout << "Merging " << sendTask << " <-> " << recvTask << " We have " << count << " and will recieve " << count_recv << " boundary links\n";
            } else {
              if(FML::ThisTask == sendTask)
                std::cout << "Merging " << sendTask << " -> " << recvTask << " We have " << count_recv << " boundary links\n";
            }
#endif
            ...
            if(merging_in_parallel){
              MPI_Sendrecv(
                  BoundaryDataToSendBack_pindex.data(),   bytes_send2, MPI_BYTE, LeftTask,  0,
                  BoundaryParticleIndex.data(),           bytes_recv2, MPI_BYTE, RightTask, 0, 
                  MPI_COMM_WORLD, &status);
              MPI_Sendrecv(
                  BoundaryDataToSendBack_fofindex.data(), bytes_send2, MPI_BYTE, LeftTask,  0,
                  BoundaryParticleRightFoFIndex.data(),   bytes_recv2, MPI_BYTE, RightTask, 0, 
                  MPI_COMM_WORLD, &status);
            } else {
              if(FML::ThisTask == recvTask)
                MPI_Send(BoundaryDataToSendBack_pindex.data(),   bytes_send2, MPI_BYTE, LeftTask,  0, MPI_COMM_WORLD);
              if(FML::ThisTask == sendTask)
                MPI_Recv(BoundaryParticleIndex.data(),           bytes_recv2, MPI_BYTE, RightTask, 0, MPI_COMM_WORLD, &status);

              if(FML::ThisTask == recvTask)
                MPI_Send(BoundaryDataToSendBack_fofindex.data(), bytes_send2, MPI_BYTE, LeftTask,  0, MPI_COMM_WORLD);
              if(FML::ThisTask == sendTask)
                MPI_Recv(BoundaryParticleRightFoFIndex.data(),   bytes_recv2, MPI_BYTE, RightTask, 0, MPI_COMM_WORLD, &status);
            }
            ...
            if(FML::ThisTask == sendTask or merging_in_parallel){
              ...
              for(size_t i = 0; i < count_recv; i++){
                ...
                if(OldFoFIndex == NewFoFIndex) continue;
...
#ifdef DEBUG_FOF
            if(FML::ThisTask == sendTask or merging_in_parallel)
              std::cout << FML::ThisTask <<  " has " << nboundary_right << " boundary particles and " << count_recv << " of them have links" << std::endl; 
#endif
          }
          if(!merging_in_parallel) MPI_Barrier(MPI_COMM_WORLD);
        }
#endif
      }
              ...
              for(size_t ii = 0; ii < np; ii++){
                ...
                if(dist2 < fof_distance2){
                  ...
                  if(id_existing_FoF_group == no_FoF_ID){
                    ...
                    if(id_existing_FoF_group == id_FoF_group) continue;
...
#pragma omp parallel for
#endif
                    for(size_t jj = 0; jj < NumPart; jj++)
                      if(particle_id_FoF[jj] == id_FoF_group || particle_id_FoF[jj] == id_existing_FoF_group) 
                        particle_id_FoF[jj] = new_FoF_id;
          ...
          MPI_Allreduce(MPI_IN_PLACE, max_FoF_id_over_tasks.data(), FML::NTasks, MPI_LONG_LONG, MPI_SUM, MPI_COMM_WORLD);
...
#pragma omp parallel for
...
#ifdef DEBUG_FOF
          std::cout << "FoFIndex on " << FML::ThisTask << " is in the range " << FoF_id_start_local << " -> " << FoF_id_start_local + nFoF-1 << "\n"; 
#endif
      ...
      template<class T, int NDIM, class FoFHaloClass = FoFHalo<T,NDIM>>
        void FriendsOfFriends(
          ...
          Ngrid = Ngrid - Ngrid % FML::NTasks;
          if(Ngrid % FML::NTasks > Ngrid/2) Ngrid *= 2;
          if(Ngrid > FoF_Ngrid_max) Ngrid = (FoF_Ngrid_max/FML::NTasks) * FML::NTasks;
          if(Ngrid < 3 * FML::NTasks) Ngrid = 3 * FML::NTasks;
          assert(1.0/Ngrid > fof_distance);
          const int Local_nx = Ngrid / FML::NTasks;
          ...
          for(auto &i : particle_id_FoF)
            if(i != no_FoF_ID) tmp.push_back(i);
          ...
          std::cout << FML::ThisTask << " has " << count << " FoF groups before merging. The number of shared groups " << nshared_groups << " Nlocal groups: " << "\n";
...
#ifdef DEBUG_FOF
            std::cout << "We have " << nshared_halos << " acceptable shared halos for a total of " << nshared_halos + non_shared_total <<  "\n";
#endif
          ...
          for(int i = 1; i < FML::NTasks; i++){
            if(FML::ThisTask == i){
              ...
              MPI_Status status;
              MPI_Recv(tmp.data(), sizeof(FoFHaloClass) * nhalosontask[i], MPI_BYTE, i,  0, MPI_COMM_WORLD, &status);
              for(auto &g : tmp){
                myhalos.push_back(g);
              }
            }
          ...
          // We are now done. Task 0 has all halos. But we should in principle send the shared halos back to where they belong
          // so all tasks just has the halos they are in charge of, i.e. the ones that fall into their domain
        }
      }
  }
