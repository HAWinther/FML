...
struct Particle {
  ...
  int get_particle_byte_size(){
    return sizeof(double)*NDIM + sizeof(volume);
  }
...
int main(){
  ...
  using WatershedBasin = FML::TRIANGULATION::WatershedBasin<Particle>;
  ...
  if(FML::ThisTask == 0) {
    for(size_t i = 0; i < watershed_groups.size(); i++){
      ...
      std::cout << i << " "
        << boxsize * watershed_groups[i].pos_barycenter[0] << " " 
        << boxsize * watershed_groups[i].pos_barycenter[1] << " " 
        << boxsize * watershed_groups[i].pos_barycenter[2] << " " 
        << boxsize * watershed_groups[i].pos_min[0] << " " 
        << boxsize * watershed_groups[i].pos_min[1] << " " 
        << boxsize * watershed_groups[i].pos_min[2] << " " 
        << volume << " " 
        << radius << " " 
        << delta_avg << " " 
        << delta_min << " " 
        << watershed_groups[i].ningroup << "\n";
    }
  }
  ...
  // };
  // d.create(part.get_particles_ptr(), part.get_npart(), buffer_fraction, random_fraction, vertex_assignment_function);
