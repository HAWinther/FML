  ...
  namespace TRIANGULATION {
    ...
    template<class T, class VD = VD_FIDUCIAL>
      class MPIPeriodicDelaunay {
        ...
        public:
          ...
          std::vector<T> & get_boundary_particles(){
            return p_boundary;
          }

          double get_dx_buffer(){
            return dx_buffer;
          }

          PeriodicDelaunay & get_delaunay_triangulation(){
            return dt;
          }
          ...
          void communicate_boundary_particles(T *p, size_t NumPart, std::vector<T> & p_to_recv){
            ...
            std::cout << "Task " << FML::ThisTask << " will send " 
              << count_left << " + " << count_right << " boundary particles\n";
#endif
            ...
            MPI_Status status;
            MPI_Sendrecv(
                &count_left,  sizeof(count_left),  MPI_CHAR, LeftTask,  0,
                &recv_right,  sizeof(recv_right),  MPI_CHAR, RightTask, 0,
                MPI_COMM_WORLD, &status);
            MPI_Sendrecv(
                &count_right, sizeof(count_right), MPI_CHAR, RightTask, 0,
                &recv_left,   sizeof(recv_left),   MPI_CHAR, LeftTask,  0,
                MPI_COMM_WORLD, &status);
#ifdef DEBUG_TESSELATION
            std::cout << "Task " << FML::ThisTask << " will recieve " 
              << recv_left << " + " << recv_right << " boundary particles\n";
#endif
            ...
            MPI_Sendrecv(
                p_to_send_left.data(),   bytes_per_particle*count_left,  MPI_CHAR, LeftTask,  0,
                p_to_recv_right.data(),  bytes_per_particle*recv_right,  MPI_CHAR, RightTask, 0,
                MPI_COMM_WORLD, &status);
            MPI_Sendrecv(
                p_to_send_right.data(),  bytes_per_particle*count_right, MPI_CHAR, RightTask, 0,
                p_to_recv_left.data(),   bytes_per_particle*recv_left,   MPI_CHAR, LeftTask,  0,
                MPI_COMM_WORLD, &status);
          ...
          void make_random_points_outside_domain(size_t nrandom, std::vector<float> & positions_random){
...
#ifdef DEBUG_TESSELATION
            std::cout << "Making random points on task " << FML::ThisTask 
              << " npts: " << nrandom_left << " " << nrandom_right << " " << nrandom_border 
              << " " << nborder << " " << xleft2 << " " << xright1 << "\n";
#endif
            ...
            if(xborder1 > 1.0) xborder1 = one*dx_buffer;
            double xborder2 = FML::xmin_domain - one*dx_buffer;
            if(xborder2 < 0.0) xborder2 = 1.0 - one*dx_buffer;
...
#elif CGAL_NDIM == 3
              Point pt(positions_random[CGAL_NDIM*i+0],positions_random[CGAL_NDIM*i+1],positions_random[CGAL_NDIM*i+2]);
#endif
              points.push_back(pt);
          ...
          void create(
            ...
            if(NumPart == 0) return;
            if(FML::NTasks == 1) random_fraction = buffer_fraction = 0.0;
            if(buffer_fraction >= 0.5 and FML::NTasks == 2) random_fraction = 0.0;
...
#ifdef DEBUG_TESSELATION
            std::cout << "Task " << FML::ThisTask << " Tesselation with " 
              << NumPart   << " (part) + " 
              << nboundary << " (boundary) + " 
              << nrandom   << " (random) = " 
              << npoints   << " (total)\n";
            ...
            if( FML::ThisTask == 0) std::cout << " Waiting for other tasks to finish" << std::endl;
            assert( dt.is_valid() );
            ...
            if( nbadcells > 0 )
              std::cout << "[MPIPeriodicDelaunay::create] Warning task " << FML::ThisTask 
                << " We have " << nbadcells << " tetrahedra that extends outside of buffer. Increase buffer\n";
          ...
          void VoronoiVolume(std::vector<double> &volumes){
            ...
            if(FML::ThisTask == 0)
              std::cout << "[MPIPeriodicDelaunay::VoronoiVolume] Volume missing in the box: " << (totvol-1)*100 << " %\n";
          }
          ...
          int num_bad_cells(){
...
#endif
            {
              ...
              if(base[0] < FML::xmin_domain or base[0] >= FML::xmax_domain) continue;
    ...
    template<class T>
      struct WatershedBasin {
        ...
        void init(double *pos){
          for(int idim = 0; idim < CGAL_NDIM; idim++){
    ...
    template<class T, class U>
      void WatershedGeneral(
        ...
        if(FML::NTasks > 1) {
          ...
          // Send quantity
          MPI_Sendrecv(
              quantity_to_send_left.data(),   sizeof(QuantityType)*count_left,  MPI_CHAR, LeftTask,  0,
              quantity_to_recv_right.data(),  sizeof(QuantityType)*recv_right,  MPI_CHAR, RightTask, 0,
              MPI_COMM_WORLD, &status);
          MPI_Sendrecv(
              quantity_to_send_right.data(),  sizeof(QuantityType)*count_right, MPI_CHAR, RightTask, 0,
              quantity_to_recv_left.data(),   sizeof(QuantityType)*recv_left,   MPI_CHAR, LeftTask,  0,
              MPI_COMM_WORLD, &status);
          ...
          for(size_t i = 0; i < nboundary; i++){
            ...
            if(v->info().min_quantity_nbor == Infinity){
              std::cout << "[WatershedGeneral] Warning boundary particle has no normal nbors " << v->point() << " " << FML::ThisTask <<"\n";
            }
          }
        }
#endif
        ...
        std::function<int(Vertex_handle,IDType)> recursive_through_vertices = [&](Vertex_handle v, IDType id) {
          if(v->info().point_type == GUARD_POINT) return 0;

          // Assign current vertex
          int count = 0;
          v->info().WatershedID = id;
          if(v->info().point_type == REGULAR_POINT) count++;
          ...
          for(auto &vnew : vertices){
            if(vnew == v) continue;
            if(vnew->info().WatershedID == id) continue;
            if(vnew->info().point_type == GUARD_POINT) continue;
            if(quantity > vnew->info().min_quantity_nbor) continue;
...
#ifdef DEBUG_TESSELATION
        std::cout << "We have " << nminima << " minima with " << ntot << " of " << NumPart << " particles tied up on task " << FML::ThisTask << "\n";
#endif
        ...
        if(FML::NTasks > 1) {
          ...
          for(int s = 0; s < 3; s++){
            ...
            MPI_Status status;
            MPI_Sendrecv(
                watershed_id_to_send_left.data(),   sizeof(IDType)*count_left,  MPI_CHAR, LeftTask,  0,
                watershed_id_to_recv_right.data(),  sizeof(IDType)*recv_right,  MPI_CHAR, RightTask, 0,
                MPI_COMM_WORLD, &status);
            MPI_Sendrecv(
                watershed_id_to_send_right.data(),  sizeof(IDType)*count_right, MPI_CHAR, RightTask, 0,
                watershed_id_to_recv_left.data(),   sizeof(IDType)*recv_left,   MPI_CHAR, LeftTask,  0,
                MPI_COMM_WORLD, &status);
            ...
            if(s == 2 and FML::ThisTask == 0) 
              std::cout << "[WatershedGeneral] Warning we merged 3 times and still not done, buffer should be larger, cannot guarantee the results are perfect\n";
          }
        }
#endif
        ...
        for(size_t i = 0; i < NumPart; i++){
          if(vs[i]->info().WatershedID != NoWatershedID) {
            ...
            std::cout << "[WatershedGeneral] Warning: on task " << FML::ThisTask 
              << " particle  " << i << " pos: " << vs[i]->point() << " has not been assigned!\n";
          }
        }
#ifdef DEBUG_TESSELATION
        ...
        if(FML::ThisTask == 0){
          ...
          if(assigned < npartotal){
            std::cout << "[WatershedGeneral] Warning we have not mange to assign all particles to their respective minima, cannot guarantee the results are perfect\n";
          }
        }
        ...
        // std::sort(watershed_groups.begin(), watershed_groups.end(), [](const auto i, const auto j){ return i.density_min > j.density_min; } );
    ...
    template<class T, class U>
      void WatershedDensity(
        ...
        std::function<void(VertexDataWatershed *, T *)> vertex_assignment_function = [](VertexDataWatershed *v, T *p){
          v->part_ptr = p ? (void *) p : nullptr;
        };
