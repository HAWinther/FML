      ...
      template<int N>
        void generate_nonlocal_gaussian_random_field_fourier(
          ...
          if(type_of_fnl == "local"){
            ...
            std::string error = "[generate_nonlocal_gaussian_random_field_fourier] Unknown fNL type " + type_of_fnl +
          ...
          FML::RANDOM::GAUSSIAN::generate_gaussian_random_field_fourier(phi_fourier, rng, DeltaPofk, fix_amplitude);
          ...
          for(auto & fourier_index : source.get_fourier_range()){
            ...
            auto value = old_source + kernel_values[1] * term1 + kernel_values[2] * term2 + kernel_values[3] * term3;
            if(kmag == 0.0){
              value = 0.0;
            }
