...
namespace FML {
  namespace GRID {
    ...
    template<int NDIM, class T>
      MPIGrid<NDIM,T>& MPIMultiGrid<NDIM,T>::get_grid(int level){
        ...
        assert_mpi(level < _Nlevel and level >= 0, 
            "[MPIMultiGrid::get_grid] Level do not exist\n");
#endif
        return _y[level];
      }
...
#ifdef BOUNDSCHECK
        assert_mpi(level < _Nlevel and level >= 0, 
            "[MPIMultiGrid::operator[]] Level do not exist\n");
#endif
...
#ifdef BOUNDSCHECK
        assert_mpi(level < _Nlevel and level >= 0, 
            "[MPIMultiGrid::set_y] Level do not exist\n");
#endif
...
#ifdef BOUNDSCHECK
        assert_mpi(level < _Nlevel and level >= 0, 
            "[MPIMultiGrid::get_N] Level do not exist\n");
#endif
    ...
    template<int NDIM, class T>
      MPIMultiGrid<NDIM,T>::MPIMultiGrid(
          MPIGrid<NDIM, T> &y, 
          int Nlevel,  
          bool periodic, 
