      ...
      class MPIGrid {
        ...
        public:
          ...
          MPIGrid(
              int N, 
              bool periodic, 
              int n_extra_slices_left = 0, 
              int n_extra_slices_right = 0);
...
#undef OPS
      ...
      inline int intlog2(int n){
        assert_mpi(n > 0, 
            "[intlog2] Can't take a log of argument <= 0\n");
        if(n == 1) return 0;
        int res = 0;
        while(n > 1){
          n /= 2;
    ...
    template<int NDIM, class T>
      IndexInt MPIGrid<NDIM,T>::index_from_globalcoord(const std::vector<int> &globalcoord) const{
        ...
        for(int idim = NDIM-1, n = 1; idim >= 0; idim--, n *= _N){
          ...
          if(idim == 0) index -= _xStartLocal * n;
        }
        return index;
      }
    ...
    template<int NDIM, class T>
      void MPIGrid<NDIM,T>::assert_index(IndexInt index) const{
        index += _NtotLocalLeft;
        assert_mpi(index >= 0, 
            "[MPIGrid::assert_index] OutOfBounds. Index corresponds to cells to the left of the extra left slices\n");
        assert_mpi(index < _NtotLocalAlloc, 
            "[MPIGrid::assert_index] OutOfBounds. Index corresponds to cells to the right of the extra right slices\n");
      }
    ...
    template<int NDIM, class T>
      void MPIGrid<NDIM,T>::send_slice_right(int ix, std::vector<T> &recv_slice){
        ...
        MPI_Sendrecv(
            ...
            recvbuf, bytes_slice, MPI_CHAR, _LeftTask,  0, 
    ...
    template<int NDIM, class T>
      void MPIGrid<NDIM,T>::send_slice_left(int ix, std::vector<T> &recv_slice){
        ...
        MPI_Sendrecv(
            ...
            recvbuf, bytes_slice, MPI_CHAR, _RightTask, 0, 
            MPI_COMM_WORLD, &status);
#endif
      }
