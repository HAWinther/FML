...
int main(){
  ...
  std::cout << "Grid has " << Nmesh << " cells per dim. Task " << FML::ThisTask << " has x-slices in the range [" << xStartLocal << " -> " << xStartLocal+nx_local << ")\n";
  ...
  // The start of the local ith-slices (i = [-nleft, ..., ] [0, 1, 2, .., nx_local -1], [nx_local, ... , nx_local + nright - 1])
  // Fetch the first slice on the right (after the main grid - this does not exist unless nright >= 1)
  int i = nx_local; 
