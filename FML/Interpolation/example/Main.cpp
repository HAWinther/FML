...
void ExamplesInterpolation(){
  ...
  if(FML::ThisTask == 0) std::cout << "Running interpolate_grid_to_particle_positions\n";
  ...
  p.create(positions.data(), positions.size(), nalloc_per_task, FML::xmin_domain, FML::xmax_domain, all_tasks_have_the_same_particles);
