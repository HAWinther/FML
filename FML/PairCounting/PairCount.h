...
namespace FML {
  namespace CORRELATIONFUNCTIONS {
    ...
    std::vector<double> AutoCorrelationEstimator(
        double *DD, 
        double *DR, 
        double *RR, 
        int nbins,
        std::string estimator);
    std::vector<double> CrossCorrelationEstimator(
        double *D1D2, 
        double *D1R2, 
        double *R1D2, 
        double *R1R2, 
        ...
        std::string estimator)
    {
      ...
      if(estimator == "LZ"){
        ...
        throw std::runtime_error("[AutoCorrelationEstimator] Unknown estimator" + estimator + ". Options: LZ, DP, HAM, HEW, PH\n");
      }
      return corr;
    }
        ...
        std::string estimator)
    {
      ...
      if(estimator == "LZ"){
        ...
        throw std::runtime_error("[CrossCorrelationEstimator] Unknown estimator. Options: LZ, HAM, DP_NO_R1, DP_NO_R2\n");
      }
      return corr;
    }
    ...
    template<class T>
      void CorrelationFunctionSimulationBox(
          std::vector<T> &particles,
          int nbins,
          double rmax,
          bool verbose){
        ...
        for(int j = 0; j < nbins; j++){
          ...
          double vol;
          if(ndim == 1) vol = 2*(r1-r0);
          if(ndim == 2) vol = M_PI*(r1*r1-r0*r0);
          if(ndim == 3) vol = 4.0*M_PI/3.0*(r1*r1*r1-r0*r0*r0);
    ...
    template<class T>
      void CorrelationFunctionSurvey(
          std::vector<T> &particles_xyz,
        ...
        for(auto &x : DD) x /= norm_DD;
        for(auto &x : RR) x /= norm_RR;
        for(auto &x : DR) x /= norm_DR;
        ...
        for(int j = 0; j < nbins; j++){
          std::cout << std::setw(10) << r[j] << " "
            << std::setw(10) << xi[j] << " " 
            << std::setw(10) << DD[j] * norm_DD << " "
            << std::setw(10) << DR[j] * norm_DR << " "
    ...
    template<class T>
      void CrossCorrelationFunctionSurvey(
          std::vector<T> &particles1_xyz,
        ...
        // Normalize pair count
        for(auto &x : D1D2) x /= norm_D1D2;
        for(auto &x : R1R2) x /= norm_R1R2;
        for(auto &x : D1R2) x /= norm_D1R2;
        for(auto &x : R1D2) x /= norm_R1D2;
          ...
          for(int ix = ngrid-1; ix >= 0; ix--){
            for(int iy = ngrid-1; iy >= 0; iy--){
              for(int iz = ngrid-1; iz >= 0; iz--){
                size_t index;
                if(ndim == 1) {index = ix; iy = iz = 0;}
                if(ndim == 2) {index = ix*ngrid + iy; iz = 0;}
                if(ndim == 3) {index = (ix*ngrid + iy)*ngrid + iz;}
          ...
          for(int iy0 = 0; iy0 <= max_iy; iy0++){
            ...
            for(int iz0 = 0; iz0 <= max_iz; iz0++){
              ...
              for(int ipart_cell = 0; ipart_cell < np_cell; ipart_cell++){
                ...
                if(ndim == 1) iz_left = iz_right = iy_left = iy_right = 0;
                if(ndim == 2) iz_left = iz_right = 0;
                ...
                for(int delta_ix = ix_left; delta_ix <= ix_right; delta_ix++){
                  ...
                  for(int delta_iy = iy_left; delta_iy <= iy_right; delta_iy++){
                    ...
                    for(int delta_iz = iz_left; delta_iz <= iz_right; delta_iz++){
                      ...
                      if(ndim == 1) index_neighbor_cell = ix;
                      if(ndim == 2) index_neighbor_cell = (ix*ngrid + iy);
                      if(ndim == 3) index_neighbor_cell = (ix*ngrid + iy)*ngrid + iz;
                      ...
                      if(!periodic)
                        if(index == index_neighbor_cell) istart_nbor_cell = ipart_cell + 1;

                      // Loop over galaxies in neighbor cells
                      for(int ipart_neighbor_cell = istart_nbor_cell; 
                          ipart_neighbor_cell < npart_neighbor_cell; ipart_neighbor_cell++){
                        ...
                        if(periodic){
                          for(int idim = 0; idim < ndim; idim++){
                            ...
                            if(dist[idim] >  1.0/2.0) dist[idim] -= 1.0;
                            if(dist[idim] < -1.0/2.0) dist[idim] += 1.0;
                          }
                        } else {
                          for(int idim = 0; idim < ndim; idim++)
...
#pragma omp critical
          {
            if(verbose) std::cout << "Processed (" << num_processed << " / " << num_max << ")\n";
            num_processed++;
          }
#endif
        }
...
#pragma omp parallel
          ...
          for(int ix = ngrid2-1; ix >= 0; ix--){
            for(int iy = ngrid2-1; iy >= 0; iy--){
              for(int iz = ngrid2-1; iz >= 0; iz--){
                size_t index;
                if(ndim == 1) {index = ix; iy = iz = 0;}
                if(ndim == 2) {index = ix*ngrid2 + iy; iz = 0;}
                if(ndim == 3) {index = (ix*ngrid2 + iy)*ngrid2 + iz;}
        ...
        if(ndim == 1) max_iz = max_iz2 = max_iy = max_iy2 = 0;
        if(ndim == 2) max_iz = max_iz2 = 0;
          ...
          for(int iy0 = 0; iy0 <= max_iy; iy0++){
            ...
            for(int iz0 = 0; iz0 <= max_iz; iz0++){
              ...
              for(int ipart_cell = 0; ipart_cell < np_cell; ipart_cell++){
                ...
                if(ndim == 1) iy2_left = iy2_right = iz2_left = iz2_right = 0;
                if(ndim == 2) iz2_left = iz2_right = 0;
                ...
                for(int delta_ix2 = ix2_left; delta_ix2 <= ix2_right; delta_ix2++){
                  ...
                  if(periodic){
                    ...
                    while(ix2 >= ngrid2) ix2 -= ngrid2;
                    while(ix2 < 0      ) ix2 += ngrid2;
                  } 
                  ...
                  for(int delta_iy2 = iy2_left; delta_iy2 <= iy2_right; delta_iy2++){
                    ...
                    if(periodic){
                      ...
                      while(iy2 >= ngrid2) iy2 -= ngrid2;
                      while(iy2 < 0      ) iy2 += ngrid2;
                    }
                    ...
                    for(int delta_iz2 = iz2_left; delta_iz2 <= iz2_right; delta_iz2++){
                      ...
                      if(periodic){
                        ...
                        while(iz2 >= ngrid2) iz2 -= ngrid2;
                        while(iz2 < 0      ) iz2 += ngrid2;
                      }

                      // Index of neighboring cell
                      int index_neighbor_cell;
                      if(ndim == 1) index_neighbor_cell = ix2;
                      if(ndim == 2) index_neighbor_cell = (ix2*ngrid2 + iy2);
                      if(ndim == 3) index_neighbor_cell = (ix2*ngrid2 + iy2)*ngrid2 + iz2;
...
#pragma omp parallel
        ...
        //========================================
        std::function<void(int,double*,T&,T&)> binning = 
          [&](int thread_id, double* dist, T &p1, T &p2){
...
#pragma omp parallel
        ...
        // Define the binning function
        //========================================
        std::function<void(int,double*,T&,U&)> binning = 
          [&](int thread_id, double* dist, T &p1, U &p2){
