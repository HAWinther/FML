        ...
        void from_LPT_potential_to_displacement_vector_scaledependent(
          ...
          for(int idim = 0; idim < N; idim++){
            if(psi[idim].get_nmesh() == 0) {
              ...
              psi.add_memory_label("FFTWGrid::from_LPT_potential_to_displacement_vector_scaledependent::Psi_" + std::to_string(idim));
            }
          }
      ...
      template<int N>
        void from_LPT_potential_to_displacement_vector(
          ...
          for(int idim = 0; idim < N; idim++){
            if(psi[idim].get_nmesh() == 0) {
              psi[idim] = FFTWGrid<N> (Nmesh, nleft, nright);
              psi[idim].add_memory_label("FFTWGrid::from_LPT_potential_to_displacement_vector::Psi_" + std::to_string(idim));
            }
          }
      ...
      template<int N>
        void compute_2LPT_potential_fourier(
          ...
          for(int i = 0; i < N; i++){
            ...
            phi_1LPT_ii[i].add_memory_label("FFTWGrid::compute_2LPT_potential_fourier::phi_1LPT_ii_" + std::to_string(i));
          }
#ifdef DEBUG_LPT
          if(FML::ThisTask == 0)
...
#ifdef DEBUG_LPT
          if(FML::ThisTask == 0)
            std::cout << "Add 0.5[(D^2 phi_1LPT)^2 - DiDi phi_1LPT^2] to real space grid containing (D^2phi_2LPT) \n";
#endif
          ...
          for(int i = 0; i < num_pairs; i++) {
            ...
            phi_1LPT_ij[i].add_memory_label("FFTWGrid::compute_2LPT_potential_fourier::phi_1LPT_ij_" + std::to_string(i));
          }
...
#ifdef DEBUG_LPT
            if(FML::ThisTask == 0)
              std::cout << "Fourier transform [DiDj phi_1LPT] to real space: " << pair+1 << " / " << num_pairs << "\n";
#endif
