...
template<int NDIM, 
  typename PosType,  bool HASPOS,  
  typename VelType,  bool HASVEL, 
  typename IDType,   bool HASID, 
  typename MassType, bool HASMASS,
  typename VolType,  bool HASVOL>
...
struct GeneralParticle {
  ...
  char data[ 
    sizeof(PosType)  * HASPOS * NDIM + 
    sizeof(VelType)  * HASVEL * NDIM +
    sizeof(IDType)   * HASID   +
    sizeof(MassType) * HASMASS +
    sizeof(VolType)  * HASVOL
  ];
  ...
  PosType *get_pos(){
    if(!HASPOS) return nullptr;
    return (PosType *) data;
  }

  PosType *get_vel(){
    if(!HASVEL) return nullptr;
  ...
  MassType get_mass(){
    if(!HASMASS) return 1.0; 
    return *(MassType *) &data[sizeof(PosType) * HASPOS * NDIM + sizeof(VelType) * HASVEL * NDIM + sizeof(IDType) * HASID];
  }
  
  VolType get_vol(){
    if(!HASVOL) return 1.0; 
    return *(VolType *) &data[sizeof(PosType) * HASPOS * NDIM + sizeof(VelType) * HASVEL * NDIM + sizeof(IDType) * HASID + sizeof(MassType) * HASMASS];
  }
  ...
  void set_mass(MassType mass){
    if(!HASMASS) return;
    MassType *mass_ptr = (MassType *) &data[sizeof(PosType) * HASPOS * NDIM + sizeof(VelType) * HASVEL * NDIM + sizeof(IDType) * HASID];
    std::memcpy(mass_ptr, &mass, sizeof(MassType) * HASMASS);
  }
  
  void set_vol(VolType vol){
    if(!HASVOL) return;
    VolType *vol_ptr = (VolType *) &data[sizeof(PosType) * HASPOS * NDIM + sizeof(VelType) * HASVEL * NDIM + sizeof(IDType) * HASID + sizeof(MassType) * HASMASS];
