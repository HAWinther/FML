...
#define FIDUCIAL_RELERR_ODE_FULL 1e-10
    ...
    typedef struct _PerturbationSystemInfo {
      ...
      _PerturbationSystemInfo(int n_ell_theta, int n_ell_theta_p, int n_ell_nu)
        : n_ell_theta(n_ell_theta)
          ...
          , n_ell_nu(n_ell_nu)
          , n_tot(n_scalar + n_ell_theta + n_ell_theta_p + n_ell_nu)
          , index_theta_start(n_scalar)
    ...
    class Perturbations {
      private:
        ...
        DVector set_ic_after_tight_coupling(
        ...
        DVector set_all_perturbations_in_tight_coupling(
            const DVector& y_tight_coupling,
            const double x,
            const double k) const;
