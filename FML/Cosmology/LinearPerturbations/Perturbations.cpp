...
namespace FML {
  namespace COSMOLOGY {
        ...
        rec(rec)
    {
      ...
      psinfo = PerturbationSystemInfo(
          n_ell_theta, 
          n_ell_theta * int(polarization),
          n_ell_nu * int(neutrinos)
          );

      psinfo_tight_coupling = PerturbationSystemInfo(
          2,
          0,
          n_ell_nu * int(neutrinos));
      ...
      for(;;){
        ...
        if(x > xstar - 0.3 and x < xstar + 1.0) delta_x_current /= 5.0;
        if(x >= -1.1*delta_x_current) break;
        x += delta_x_current;
        ...
        const double k) const
    {
      ...
      //tcb::span<const double> Theta_tc  (y_tight_coupling.data() + psinfo_tight_coupling.index_theta_start,   n_ell_theta_tc);
      //tcb::span<const double> Nu_tc     (y_tight_coupling.data() + psinfo_tight_coupling.index_nu_start,      n_ell_nu_tc);
      //=============================================================================
    ...
    double Perturbations::get_tight_coupling_time(const double k) const
    {
      ...
      if (cosmo->get_OmegaB() == 0.0) {
        if(verbose) std::cout << "k: " << k * Constants.Mpc << " z: " << exp(-x)-1 << "\n";  
        return x;
      }
    ...
    DVector Perturbations::set_all_perturbations_in_tight_coupling(
        const DVector& y_tight_coupling,
        const double x,
        const double k) const
    {
      ...
      for (int ell = 0; ell < n_ell_theta; ell++) {
        if (ell < n_ell_theta_tc) {
          ...
          if (ell == 2) {
            ...
            y[index_theta_start + ell] = -ell / (2.0 * ell + 1) * ckoverHpdtaudx * y[index_theta_start + ell - 1];
          }
        }
      }
      ...
      for (int ell = 0; ell < n_ell_theta_p; ell++) {
        if (ell == 0) {
          ...
          y[index_theta_p_start + ell] = -ell / (2.0 * ell + 1) * ckoverHpdtaudx * y[index_theta_p_start + ell - 1];
        }
      }
    ...
    void Perturbations::integrate_perturbations()
    {
      ...
      std::cout << "Integrate " << n_k_total << " wavenumbers in the range [" 
        << k_min * Constants.Mpc << " , " << k_max * Constants.Mpc << "]\n";
...
#pragma omp parallel for schedule(dynamic, 1)
      ...
      for (int ik = 0; ik < n_k_total; ik++) {
        ...
        ODESolver tight_coupling_ode(
            FIDUCIAL_HSTART_ODE_TIGHT, 
            FIDUCIAL_ABSERR_ODE_TIGHT, 
            FIDUCIAL_RELERR_ODE_TIGHT);
        ...
        ODESolver full_ode(
            FIDUCIAL_HSTART_ODE_FULL, 
            FIDUCIAL_ABSERR_ODE_FULL, 
            FIDUCIAL_RELERR_ODE_FULL);
...
#define USE_JACOBIAN
      ...
      for (int ik = 0; ik < n_k_total; ik++) {
        ...
        for (int ix = 0; ix < n_x_total; ix++) {
          ...
          const double ckoverHp = Constants.c * k / cosmo->Hp_of_x(x);
          Psi_array[index] = -get_Phi(x, k) - 12.0 / (ckoverHp * ckoverHp) 
            * (cosmo->get_OmegaR(x) * Theta2 + cosmo->get_OmegaNu(x) * Nu2);
      ...
      for (int ik = 0; ik < n_k_total; ik++) {
        ...
        for (int ix = 0; ix < n_x_total; ix++) {
          ...
          zeta_array[index] = Phi + (-ckoverHp*ckoverHp/3.0*Phi + 0.5*drho_source) / (1.5 * (1+weff));
        }
      }
      ...
      dzetadx_array[index] = -2.0/3.0/(1+weff) * PprimeoverRhoprime * Phi * pow(Constants.c * k / Hp,2) + PnadTerm/(1+weff);
      }
      }
    ...
    void Perturbations::compute_source_functions()
    {
      ...
      for (int ix = 0; ix < n_x_total; ix++) {
        ...
        for (int ik = 0; ik < n_k_total; ik++) {
          ...
          const double polarization_term = 3.0 / (4.0 * ckoverHp * ckoverHp) * (
              (ddgddx * Pi + 2.0 * dgdx * dPidx + g * ddPiddx) 
              + 3.0 * dHpdxHp * (dgdx * Pi + g * dPidx) 
    ...
    void Perturbations::output_perturbations(const double k,
        const std::string filename) const
    {
      ...
      auto print_data = [&](const double x) {
        ...
        fp << get_Source_E(x, k) * FML::MATH::j_ell(50, arg) * sqrt((50. + 2) * (50. + 1) * (50. + 0) * (50. - 1)) << " ";
        fp << get_Source_E(x, k) * FML::MATH::j_ell(500, arg) * sqrt((500. + 2) * (500. + 1) * (500. + 0) * (500. - 1)) << " ";
    ...
    void Perturbations::output_transfer(const double x,
        const std::string filename) const
    {
      std::ofstream fp(filename.c_str());
      const int npts = 1000;
    ...
    int Perturbations::rhs_tight_coupling_ode(double x,
        double k,
        const double* y,
        double* dydx)
    {
      ...
      for (int ell = 0; ell < n_ell_nu; ell++) {
        ...
        // Add sources
        if(ell == 0) dNudx[ell] += - dPhidx;
        if(ell == 1) dNudx[ell] += ckoverHp / 3.0 * Psi;
      }

      return GSL_SUCCESS;
    }

    int Perturbations::rhs_jacobian_full(double x,
        double k,
        const double* y,
        double* dfdy,
        double* dfdt)
    {
      ...
      dfdt_Phi = dPsidt + 2.0 * dlogHpdx * ckoverHp * ckoverHp * Phi / 3.0 + PhiH0H0term 
        * (+(-1.0 - 2 * dlogHpdx) * (OmegaCDM * delta_cdm * a + OmegaB * delta_b * a) + (-2.0 - 2 * dlogHpdx) * 4.0 * (OmegaR * Theta[0] + OmegaNu * Nu0));
      ...
      for (int ell = 0; ell < psinfo.n_ell_theta; ell++) {
        if (ell == 0) {
          ...
          dfdt_Theta[1] = -dlogHpdx * ckoverHp / (2.0 * ell + 1) * (ell * Theta[0] - (ell + 1) * Theta[2] + Psi) + ckoverHp / 3.0 * dPsidt + ddtauddx * (Theta[ell] + v_b / 3.0);
        } else if (ell < psinfo.n_ell_theta - 1) {
          dfdt_Theta[ell] = -dlogHpdx * ckoverHp / (2.0 * ell + 1) * (ell * Theta[ell - 1] - (ell + 1) * Theta[ell + 1]) + ddtauddx * (Theta[ell] - (ell == 2 ? Pi / 10.0 : 0.0));
        } else {
          dfdt_Theta[ell] = -dlogHpdx * ckoverHp * Theta[ell - 1] - (ell + 1) * doneoveretaHpdx * Theta[ell] + ddtauddx * Theta[ell];
        }
      }
      ...
      for (int ell = 0; ell < psinfo.n_ell_theta_p; ell++) {
        if (ell == 0) {
          ...
          dfdt_Theta_p[ell] = -dlogHpdx * ckoverHp / (2.0 * ell + 1) * (ell * Theta_p[ell - 1] - (ell + 1) * Theta_p[ell + 1]) + ddtauddx * (Theta_p[ell] - (ell == 2 ? Pi / 10.0 : 0.0));
        } else {
          dfdt_Theta_p[ell] = -dlogHpdx * ckoverHp * Theta_p[ell - 1] - (ell + 1) * doneoveretaHpdx * Theta_p[ell] + ddtauddx * Theta_p[ell];
        }
      }
      ...
      for (int ell = 0; ell < psinfo.n_ell_nu; ell++) {
        if (ell == 0) {
          ...
          dfdt_Nu[ell] = -dlogHpdx * ckoverHp / (2.0 * ell + 1) * (ell * Nu[ell - 1] - (ell + 1) * Nu[ell + 1]);
        } else {
      ...
      for (int ell = 0; ell < psinfo.n_ell_nu; ell++) {
        ...
        if (ell == 0) {
          ...
          cur[psinfo.index_nu_start + 2] = -(ell + 1) / (2.0 * ell + 1) * ckoverHp - ckoverHp * Psiterm * OmegaNu;
          cur[psinfo.index_theta_start + 2] = -ckoverHp * Psiterm * OmegaR;
        } else if (ell < psinfo.n_ell_nu - 1) {
    ...
    int Perturbations::rhs_full_ode(double x,
        double k,
        const double* y,
        double* dydx)
    {
      ...
      ddelta_bdx   = ckoverHp * v_b   - 3.0 * dPhidx;
      dv_cdmdx     = -v_cdm - ckoverHp * Psi;
      dv_bdx       = -v_b   - ckoverHp * Psi + ckoverHp * sound_speed_squared * delta_b + dtaudx * R * (3.0 * Theta[1] + v_b);
      ...
      for (int ell = 0; ell < n_ell_theta; ell++) {
        ...
        // Add sources
        if(ell == 0) dThetadx[ell] += -dtaudx * Theta[ell] - dPhidx;
        if(ell == 1) dThetadx[ell] +=  dtaudx * v_b / 3.0 + ckoverHp / 3.0 * Psi;
        if(ell == 2) dThetadx[ell] += -dtaudx * Pi / 10.0;
      }
      ...
      for (int ell = 0; ell < n_ell_theta_p; ell++) {
        ...
        // Add sources
        if(ell == 0) dTheta_pdx[ell] += -dtaudx * Pi / 2.0;
        if(ell == 1) dTheta_pdx[ell] += 0.0;
        if(ell == 2) dTheta_pdx[ell] += -dtaudx * Pi / 10.0;
      }
    ...
    //====================================================
    double Perturbations::get_Source_SW_T(const double x, const double k) const
    {
      return SW_spline(x, k);
    }
    double Perturbations::get_Source_ISW_T(const double x, const double k) const
    {
      return ISW_spline(x, k);
    }
    double Perturbations::get_Source_VEL_T(const double x, const double k) const
    {
      return VEL_spline(x, k);
    }
    double Perturbations::get_Source_POL_T(const double x, const double k) const
    {
      return POL_spline(x, k);
    }
    ...
    {
      if(psinfo.n_ell_theta_p == 0) return 0.0;
      if (ell >= 3 and not pert_spline_all_ells) {
        return 0.0;
      }
    ...
    {
      if(psinfo.n_ell_nu == 0) return 0.0;
      if (ell >= 3 and not pert_spline_all_ells) {
        return 0.0;
      }
    ...
    double Perturbations::get_transfer_zeta(const double x, const double k) const
    {
      return zeta_spline(x, k);
    }
    ...
    double Perturbations::get_transfer_Delta_cb(const double x, const double k) const
    {
      const double OmegaB = cosmo->get_OmegaB();
      const double OmegaCDM = cosmo->get_OmegaCDM();
      return (get_transfer_Delta_b(x,k) * OmegaB + get_transfer_Delta_cdm(x,k) * OmegaCDM) / (OmegaB + OmegaCDM);
    }
    ...
    {
      return 4.0 * (get_Theta(x,k,0) + 3.0*cosmo->Hp_of_x(x) * get_Theta(x,k,1) / (Constants.c * k)) / (k*k);
    }
    ...
    double Perturbations::get_transfer_Delta_Rtot(const double x, const double k) const
    {
      const double OmegaR = cosmo->get_OmegaR();
      const double OmegaNu = cosmo->get_OmegaNu();
      return (get_transfer_Delta_R(x,k) * OmegaR + get_transfer_Delta_Nu(x,k) * OmegaNu) 
        / (OmegaR + OmegaNu);
    }
    ...
    double Perturbations::get_transfer_Phi(const double x, const double k) const
    {
      return Phi_spline(x, k);
    }

    double Perturbations::get_transfer_Psi(const double x, const double k) const
    {
      return Psi_spline(x, k);
    }
    ...
    double Perturbations::get_kmin() const
    {
      return k_min;
    }
    double Perturbations::get_kmax() const
    {
      return k_max;
    }

    double Perturbations::get_x_start() const
    {
      return x_start;
    }

    double Perturbations::get_x_end() const
    {
      return x_end;
    }
  }
