  ...
  namespace COSMOLOGY {
    ...
    void RecombinationHistory::info() const
    {
      ...
      if (tau_of_x_spline) {
        ...
        std::cout << "Last scattering (tau = 1.0) with Saha      = " << exp(-x_star_saha) - 1 << " Xe: " << Xe_of_x_saha(x_star_saha) << "\n";
        std::cout << "Last scattering (dgdx = 0.0)       z       = " << exp(-x_star2) - 1 << "\n";
        std::cout << "Last scattering (tau = 1.0)        zstar   = " << exp(-x_star) - 1 << " Xe: " << Xe_of_x(x_star) << "\n";
    ...
    std::pair<double, double>
      RecombinationHistory::electron_fraction_from_saha_equation_without_helium(
          double x) const
      {
    ...
    std::pair<double, double>
      RecombinationHistory::electron_fraction_from_saha_equation_with_helium(
          double x) const
      {
        const double a = exp(x);
    ...
    void RecombinationHistory::solve_number_density_electrons()
    {
      ...
      for (int i = 0; i < npts; i++) {
        ...
        if (Xe_saha_arr[i] >= Xe_saha_limit) {
          ...
          ODESolver peebles_Xe_ode(FIDUCIAL_HSTART_ODE_PEEBLES,
              FIDUCIAL_ABSERR_ODE_PEEBLES,
              FIDUCIAL_RELERR_ODE_PEEBLES);
      ...
      for (int j = 0; j < npts; j++) {
        cs2_baryon_array[j] = (Constants.k_b * Tb_spline(x_array[j])) / (Constants.m_H * Constants.c * Constants.c)
      ...
      // Find recombination redshift (NB: this has to be the Xe without reionization for the binary search to work)
      try{
    ...
    int RecombinationHistory::rhs_peebles_ode(double x,
        const double* y,
        double* dydx)
    {
    ...
    void RecombinationHistory::solve_for_optical_depth_tau()
    {
      ...
      // Solve the ODE
      ODESolver tau_ode(FIDUCIAL_HSTART_ODE_TAU, FIDUCIAL_ABSERR_ODE_TAU,
          FIDUCIAL_RELERR_ODE_TAU);
      ...
      ddgddx_tilde_of_x_spline.create(x_array, ddgddx_tilde_array,
          "ddgddx_tilde_of_x");
    ...
    void RecombinationHistory::solve_extra(){
      ...
      ODESolver r_ode(FIDUCIAL_HSTART_ODE_TAU, FIDUCIAL_ABSERR_ODE_TAU,
          FIDUCIAL_RELERR_ODE_TAU);
      ...
      ODESolver kd_ode(FIDUCIAL_HSTART_ODE_TAU, FIDUCIAL_ABSERR_ODE_TAU,
          FIDUCIAL_RELERR_ODE_TAU);
    ...
    {
      return tau_of_x_saha_spline(x);
    }

    double RecombinationHistory::tau_of_x_saha_noreion(double x) const
    {
      return tau_of_x_saha_noreion_spline(x);
    }
    ...
    {
      return dgdx_tilde_of_x_spline(x);
    }

    double RecombinationHistory::ddgddx_tilde_of_x(double x) const
    {
      return ddgddx_tilde_of_x_spline(x);
    }
    ...
    double RecombinationHistory::Xe_of_x_noreion(double x) const
    {
    ...
    double RecombinationHistory::Xe_of_x_saha_noreion(double x) const
    {
    ...
    // Baryon temperature
    double RecombinationHistory::get_Tbaryon(double x) const {
      return Tb_spline(x);
    }
    ...
    double RecombinationHistory::get_baryon_sound_speed_squared(double x) const {
      // Simpler approx:
      // 4.0 / 3.0 * Constants.k_b * cosmo->get_TCMB() * (1.0 - rec->get_Yp()) / (a * Constants.m_H * Constants.c * Constants.c);
    ...
    double RecombinationHistory::get_x_start_rec_array() const
    {
      return x_start_rec_array;
    }
    ...
    double RecombinationHistory::get_xstar() const
    {
      return x_star;
    }

    double RecombinationHistory::get_xdrag() const
    {
      return x_drag;
    }
