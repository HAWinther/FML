...
namespace FML {
  namespace COSMOLOGY {
    ...
    BackgroundCosmology::BackgroundCosmology(const ParameterMap& p)
    {
      ...
      double OmegaRh2 = 2.0 * (M_PI * M_PI / 30.0) * pow(Constants.k_b * TCMB / Constants.hbar, 4) * Constants.hbar / pow(Constants.c, 5) / rhoc0_over_h2;
      ...
      const double min_OmegaKh2 = -pow(OmegaLambdah2 * 27/4. * (OmegaCDMh2+OmegaBh2) * (OmegaCDMh2+OmegaBh2),0.33333);
      if(OmegaKh2 < min_OmegaKh2) {
        throw std::runtime_error("OmegaKh2 = " + std::to_string(OmegaKh2) + " is too negative (must be > " 
            + std::to_string(min_OmegaKh2) + " ). The Hubble factor will be zero at some point\n");
      }
    ...
    void BackgroundCosmology::info() const
    {
      ...
      if (eta_of_x_spline) {
        std::cout << "The age of the universe    "
          << get_cosmic_time(0.0) / Constants.Gyr << " Gyr\n";
        std::cout << "Conformal time today       "
          << eta_of_x(0.0) / (Constants.Gyr * Constants.c)
          << " Gyr\n";
        std::cout << "Conformal distance today   "
          << eta_of_x(0.0) / Constants.Gpc << " Gpc\n";
      }
      std::cout << "============================================\n";
      std::cout << "\n";
    }
    ...
    void BackgroundCosmology::compute_background(){
      ...
      auto Hp_function = [&](double x) { 
        return exp(x) * H_function(x); 
      };
      auto dHdx_function = [&](double x) {
        return 1.0 / (2.0 * H_function(x)) * H0 * H0 * (-2 * OmegaK * exp(-2 * x) - 3 * OmegaM * exp(-3 * x) - 4 * OmegaRtot * exp(-4 * x));
      };
      auto dHpdx_function = [&](double x) {
        return 1.0 / (2.0 * Hp_function(x)) * H0 * H0 * (2 * OmegaLambda * exp(2 * x) - OmegaM * exp(-x) - 2 * OmegaRtot * exp(-2 * x));
      };
      auto w_function = [&](double x) {
        return (OmegaRtot * exp(-4 * x)/3.0 - OmegaLambda -OmegaK * exp(-2 * x)/3.0) / pow(H_function(x)/H0,2);
      };
    ...
    void BackgroundCosmology::compute_conformal_time(){
      ...
      if( OmegaRtot > 0.0 )
        eta_initial = { exp(x_array[0]) / sqrt(OmegaRtot),
          exp(2.0 * x_array[0]) / 2.0 / sqrt(OmegaRtot) };

      // Solve the ODE
      ODESolver eta_ode(FIDUCIAL_COSMO_HSTART_ODE_ETA,
          FIDUCIAL_COSMO_HSTART_ODE_ETA,
          FIDUCIAL_COSMO_RELERR_ODE_ETA);
    ...
    {
      return H_spline.deriv_x(x);
    }

    double BackgroundCosmology::ddHddx_of_x(double x) const
    {
      return H_spline.deriv_xx(x);
    }

    double BackgroundCosmology::dHpdx_of_x(double x) const
    {
      return Hp_spline.deriv_x(x);
    }

    double BackgroundCosmology::ddHpddx_of_x(double x) const
    {
      return Hp_spline.deriv_xx(x);
    }

    double BackgroundCosmology::eta_of_x(double x) const
    {
      if(x == 0.0) return eta0;
    ...
    double BackgroundCosmology::get_K() const
    {
      return K;
    }
    ...
    double BackgroundCosmology::get_Tnu(double x) const
    {
      return pow(4.0 / 11.0, 1.0 / 3.0) * get_TCMB(x);
    }
    ...
    std::string BackgroundCosmology::get_name() const
    {
      return name;
    }
    ...
    {
      return cosmic_time_of_x_spline(x);
    }
