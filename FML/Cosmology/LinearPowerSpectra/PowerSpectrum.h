...
#define FIDUCIAL_RELERR_ODE_CELL 1e-12
    ...
    class PowerSpectrum {
      ...
      public:
        ...
        DVector2D line_of_sight_integration_single(
            DVector& x_array,
            DVector& k_array,
        ...
        double get_j_ell(int ell, double x){
          return j_ell_splines[int(index_of_ells_spline(ell))](x);
        }
...
#ifdef USE_FFTW
    std::pair<DVector,DVector> correlation_function_single_fftw(
        std::function<double(double)>& Delta_P, 
        double rmin = 1.0 * Constants.Mpc, 
        double rmax = 512.0 * Constants.Mpc, 
        int ngrid_min = 8192);

    std::pair<DVector,DVector> correlation_function_single_fftlog(
        std::function<double(double)>& Delta_P, 
