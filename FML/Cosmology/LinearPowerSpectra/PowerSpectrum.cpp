  ...
  namespace COSMOLOGY {
      ...
      pert(pert)
    {
      ...
      if(sample_all_ells) {
        ...
        for(int ell = 2; ell < ell_max;){
          ...
          // Low ell sampling
          if(ell < 10)        ell += 1;
          else if(ell < 20)   ell += 2;
          else if(ell < 50)   ell += 5;

          // Acoustic peak sampling
          else if(ell < 100)  ell += 10;
          else if(ell < 300)  ell += 20;
          else if(ell < 1000) ell += 25;
    ...
    double PowerSpectrum::get_power_spectrum(double x, double k, std::string type) const
    {
      ...
      static auto bbks_fit = [](double k){
        ...
        return log(1.0 + 0.171*arg)/(0.171*arg)*pow(
            1+0.284*arg+pow(1.18*arg,2) + pow(0.399*arg,3) + pow(0.490*arg,4),-0.25);
      };
      ...
      if(k_true < kmin or k_true > kmax){
        ...
        if(k_true > kmax){
          ...
          if( ! radiation ) Delta *= bbks_fit(k_true) / bbks_fit(kmax); 
        }
      }  
    ...
    std::pair<DVector, DVector> PowerSpectrum::get_power_spectrum_array(
        double x, int npts, std::string type) const
    {
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        [[maybe_unused]] std::function<double(double, double)>& aux_norm)
    {
      ...
      for(size_t i = 0; i < x_array.size(); i++){ 
        ...
        if(x_array[i] > -3.0) g_relevant[i] = 1;
      }
...
#pragma omp parallel for schedule(dynamic, 1)
      ...
      for (size_t ik = 0; ik < k_array.size(); ik++) {
        ...
        ODESolver los_ode(FIDUCIAL_HSTART_ODE_LOS, FIDUCIAL_ABSERR_ODE_LOS,
            FIDUCIAL_RELERR_ODE_LOS);
        ...
        for (size_t ix = 1; ix < x_array.size(); ix++) {
          if(g_relevant[ix] == 0 and x_array[ix] < -4.0) continue;
          ...
          for (int i = 0; i < nells; i++) {
            if(k < kcut[i]) continue;
            if(ells[i] > 30.0 and g_relevant[ix] == 0) continue;
            const Spline& jell_spline = j_ell_splines[i];
            const double jell = jell_spline(arg);
    ...
    void PowerSpectrum::output_correlation_function(double x, std::string filename) const{
      if(!compute_corr_function) return;
    ...
    void PowerSpectrum::solve()
    {
      ...
      //=========================================================================
      if(compute_corr_function) compute_all_correlation_functions();
      ...
      if (Nu_ell_of_k_spline) {
        ...
        std::function<double(double,int)> D_ell_NN_integrand = [&](double k, int ell){
          ...
          const double Neff_ini = pert->get_Nu(xini, k, 0) + pert->get_Psi(xini, k) + pert->get_Nu(xini, k, 2)/4.0;
      ...
      if (lens_ell_of_k_spline) {
        ...
        for(size_t i = 0; i < ells.size(); i++) cell_LL[i] *= ells[i] * (ells[i]+1) * ells[i] * (ells[i]+1);
      ...
      if (lens_ell_of_k_spline and thetaT_ell_of_k_spline) {
        ...
        for(size_t i = 0; i < ells.size(); i++) cell_TL[i] *= 1.0;
    ...
    void PowerSpectrum::output_matter_power_spectrum(double x,
    ...
    double PowerSpectrum::get_cell_TT(double ell) const
    {
      return cell_TT_spline(ell);
    }
    double PowerSpectrum::get_cell_TE(double ell) const
    {
      return cell_TE_spline(ell);
    }
    double PowerSpectrum::get_cell_EE(double ell) const
    {
      return cell_EE_spline(ell);
    }
    double PowerSpectrum::get_cell_LL(double ell) const
    {
      return cell_LL_spline(ell);
    }
    double PowerSpectrum::get_cell_NN(double ell) const
    {
      return cell_NN_spline(ell);
    }
    double PowerSpectrum::get_corr_func_CDM(double x, double r) const{
      return xi_CDM_spline(x, r);
    }
    double PowerSpectrum::get_corr_func_B(double x, double r) const{
      return xi_B_spline(x, r);
    }
    double PowerSpectrum::get_corr_func_R(double x, double r) const{
      return xi_R_spline(x, r);
    }
    double PowerSpectrum::get_corr_func_Nu(double x, double r) const{
      return xi_Nu_spline(x, r);
    }
    double PowerSpectrum::get_corr_func_M(double x, double r) const{
      return xi_M_spline(x, r);
    }
    ...
    double PowerSpectrum::get_corr_func(double x, double r, std::string type) const{
      if(type == "M")    return get_corr_func_M(x,r);
      if(type == "CDM")  return get_corr_func_CDM(x,r);
      if(type == "B")    return get_corr_func_B(x,r);
      if(type == "CB")   return get_corr_func_CB(x,r);
      if(type == "R")    return get_corr_func_R(x,r);
      if(type == "Nu")   return get_corr_func_Nu(x,r);
      if(type == "Rtot") return get_corr_func_Rtot(x,r);
