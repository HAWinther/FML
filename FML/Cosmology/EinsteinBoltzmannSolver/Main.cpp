...
int main() {
    ...
    // Accuracy settings
    param_map["pert_integration_nk_per_logint"] =
        25; // Number of k-points per logarithmic interval, 100 for high accuracy
    param_map["pert_spline_all_ells"] =
        false; // Spline all the Theta_ell(k,x), Nu_ell(k,x), etc. multipoles or just the first 0,1,2
    param_map["pert_x_initial"] = -15.0; // When to start the integration x = log(aini)
    param_map["pert_delta_x"] =
        0.05; // How many points to store in the integration till today, every deltax = deltalog(a)
