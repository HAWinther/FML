...
int main(){
  ...
  //==============================================
  if(FML::ThisTask == 0) std::cout << "Print non-zero fourier coefficients (Expecting only |k|=" << n <<"):\n";
  for(auto&& index : grid.get_fourier_range()){
    ...
    if(std::abs(value) > 1e-5){
      ...
      std::cout << " ==> " << value << "\n";
      //FML::assert_mpi(std::fabs( std::sqrt(knorm2)/(2.0*M_PI) - n) < 1e-5, "Non-zero fourier coefficient with |k| != n\n");
    }
  ...
  //==============================================
  if(FML::ThisTask == 0) std::cout << "Compare solution to analytical solution:\n";
  for(auto&& index : grid.get_real_range()){
    // The (local) coordinates of the cell
    auto coord = grid.get_coord_from_index(index);
    ...
    if(std::abs(value - value_analytic) > 1e-10)
      std::cout << "Solution: " << value << " Analytic: " << value_analytic << " Error: " << std::abs(value - value_analytic) << "\n";
  }
  if(FML::ThisTask == 0) std::cout << "(No output above means the solution matches the analytical one to < 1e-10)\n";
