...
namespace FML {
  namespace SOLVERS {
    namespace MULTIGRIDSOLVER {
      ...
      using namespace FML::GRID;

      // Simpler syntax for user-provided functions
      ...
      template<int NDIM, class T>
        class MultiGridSolver {
          private:
            ...
            int _ngs_first_step  = 10;     // Number of NGS sweeps the very first time (in case the guess is not very good)
          ...
          public:
            ...
            void solve(
                MultiGridFunction<NDIM,T> & Equation,
                MultiGridConvCrit & ConvergenceCriterion)
            {
              _Equation = Equation;
            ...
            MultiGridConvCrit ConvergenceCriterionResidual = [=](double rms_residual, double rms_residual_ini, int step_number){
              if(rms_residual < _eps_converge) return true;
              return false;
            };
            // Error on domain grid < epsillon (residual relative to residuals after presweeps)
            MultiGridConvCrit ConvergenceCriterionError = [=](double rms_residual, double rms_residual_ini, int step_number){
              double err = rms_residual_ini != 0.0 ? rms_residual / rms_residual_ini : 1.0;
              if(err < _eps_converge) return true;
              return false;
            };

            // Example implementation of a PDE
            // Poisson equation with source s.t. the solution is f(x_1,x_2,...) = Sum_i sin(2 pi x_i)
            MultiGridFunction<NDIM,T> TestEquation = [](MultiGridSolver<NDIM,T> *sol, int level, IndexInt index) {
      ...
      //================================================
      template<int NDIM, class T>
        MultiGridSolver<NDIM,T>::MultiGridSolver(int N, int Nlevels, bool verbose, bool periodic, int n_extra_slices_left, int n_extra_slices_right) :
          _N(N), 
          _periodic(periodic),
          _verbose(verbose)
      {
        ...
        if(_periodic){
          ...
          for(IndexInt i = 0; i < _NtotLocal; i++){
            ...
            double mask = 1.0;
            for(int idim = 0; idim < NDIM; idim++){
              if(coord[idim] <   1  ) mask = -1.0;
              if(coord[idim] >= _N-1) mask = -1.0;
            }
            if(coord[0] >= _N/2 && coord[1] >= _N/2) mask = -1.0;
            mainmask[i] = mask;
          }
        }
...
#pragma omp parallel
      ...
      inline const double* derivative_stencil_weights_deriv1(int order){
        ...
        static const double Coeff_4[] = { 1/280., -4/105.,  1/5.  , -4/5.  ,  0  , 4/5. , -1/5.   ,  4/105., -1/280. };
      ...
      inline const double* derivative_stencil_weights_deriv2(int order){
        ...
        static const double Coeff_4[] = { -1/560., 8/315.,  -1/5. , 8/5.  , -205/72., 8/5., -1/5.  ,  8/315.,  -1/560. };
        static const double* Coeff[] = {Coeff_1, Coeff_2, Coeff_3, Coeff_4};
        return Coeff[order-1];
      }
...
#ifdef USE_MASK
            if(_bmask[level][i] <= 0.0) continue;
#endif
            res[i] = (_Equation(this, level, i).first) * T(-1.0);
            if(level > 0) res[i] += _source[level][i];
          }
      ...
      template<int NDIM, class T>
        bool MultiGridSolver<NDIM,T>::is_converged(){
          ...
          if(_verbose and converged) {
            std::cout << std::endl;
            std::cout << "    The solution has converged res = " << _rms_res << " err = " << err << " istep = " << _istep_vcycle << "\n" << std::endl;
          } 
          if (_verbose and not converged) {
            std::cout << "    The solution has not yet converged res = " << _rms_res << " err = " << err << " istep = " << _istep_vcycle << "\n";
          } 
...
#pragma omp parallel for 
      ...
      template<int NDIM, class T>
        void MultiGridSolver<NDIM,T>::make_prolongation_array(MPIGrid<NDIM,T>& f, MPIGrid<NDIM,T>& Rf, MPIGrid<NDIM,T>& df){
...
#pragma omp parallel for
      ...
      // Assumed: index_list is the same as produced by get_neighbor_gridindex
      template<int NDIM, class T>
        T MultiGridSolver<NDIM,T>::get_derivLaplacian(int level, const std::vector<IndexInt>& index_list) {
          const double h = 1.0 / double(get_N(level));
