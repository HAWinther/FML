...
For the MG models (so far for f(R), symmetron) one has all the options of 1) simulating using linear field equations (no screening) 2) using fast approximate screening method or 3) solving the exact equations using a multigridsolver (slow).
