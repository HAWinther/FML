...
class BoxReplicas {
  ...
  public:
    ...
    void read_parameters(ParameterMap & param) {
        ...
        if (FML::ThisTask == 0) {
            std::cout << "# Read in Lightcone_Replicas.h:\n";
    ...
    void init(double a_init, double R_init) {
        ...
        if (not use_replicas and R_init > 1.0) {
            if (FML::ThisTask == 0) {
                throw std::runtime_error("We do not use replicas, but R_init / boxsize > 1 so lightcone quadrant will "
                                         "not be complete. Reduce z_init, increase boxsize or add using replicas");
            }
        }
        ...
        if (FML::ThisTask == 0) {
            ...
            for (int idim = 0; idim < NDIM; idim++) {
                std::cout << "# For idim = " << idim << " we have replicas from -" << n_per_dim_left[idim] << " -> +"
                          << n_per_dim_right[idim] << " boxes\n";
    ...
    size_t get_coord_of_replica(const std::array<int, NDIM> & index) {
        size_t coord = 0;
        for (int idim = 0; idim < NDIM; idim++)
            coord = coord * (n_per_dim_left_alloc[idim] + n_per_dim_right_alloc[idim] + 1) +
                    (index[idim] + n_per_dim_left_alloc[idim]);
        return coord;
    }
    ...
    void flag(double r_old, double r_new) {
        ...
        for (size_t i = 0; i < n_replicas_total; i++) {
            ...
            if (in_use_flag[coord] == COMPLETELY_OUTSIDE_LC)
                continue;

            // Compute minimum and maximum distance from observer to the box
            Point corner_point;
            for (int idim = 0; idim < NDIM; idim++)
                corner_point[idim] = index[idim];
    ...
    std::pair<double, double> minmax_distance_from_observer_to_box(Point & corner_point, Point & observer) {
        ...
        for (int idim = 0; idim < NDIM; idim++) {
            inside_per_dim[idim] =
                (corner_point[idim] <= observer[idim] and (corner_point[idim] + 1.0) >= observer[idim]);
        }
    ...
    bool is_flagged_in_use(size_t coord) { return in_use_flag[coord] == INTERSECTING_LC; }
