...
class GravityModel {
  public:
    ...
    Spline2D D_2LPT_of_logkoverH0_loga{"[D2LPT(log(k/H0),log(a)) was not yet created]"};
    ...
    virtual void output(std::string filename, double koverH0) {
        ...
        fp << "#   a   GeffG(a,k)    D1(a,k)  D2(a,k)   D3a(a,k)   D3b(a,k)  (source terms)  (Output for k "
              "= "
           << k << " h/Mpc)\n";
    ...
    virtual double source_factor_1LPT([[maybe_unused]] double a, [[maybe_unused]] double koverH0 = 0) const {
        ...
        if (OmegaMNu > 0.0) {
            if (transferdata) {
                ...
                const double OmegaCB = OmegaM - OmegaMNu;
                ...
                factor = OmegaCB / OmegaM + OmegaMNu / OmegaM * T_nu / T_cb;
            }
        }
        return factor;
    };
    ...
    virtual void compute_growth_factors() {
        ...
        auto solve_growth_equations = [&](double koverH0) -> std::tuple<DVector, DVector, DVector, DVector> {
            ...
            std::vector<double> yini{D1_ini, dD1dx_ini, D2_ini, dD2dx_ini, D3a_ini, dD3adx_ini, D3b_ini, dD3bdx_ini};
