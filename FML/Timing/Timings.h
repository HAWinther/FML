      ...
      public:
        ...
        void PrintAllTimings(){
          ...
          for (const auto& t : elapsed_time_sec) {
            std::cout << "Total elapsed time for [" << std::setw(35) << t.first
              << "]: " << std::setw(10) << t.second << " sec\n";
          }
          std::cout << "============================================\n";
          std::cout << "\n";
        }
        ...
        // End timing
        double EndTiming(const char* name, bool print = false){
          return EndTiming(std::string(name), print);
        }
        ...
        double EndTiming(std::string name, bool print = false){
          ...
          if (print) {
            if (tot_time_sec == 0.0) {
              if (time_sec == 0.0) {
                std::cout << "Can't print elapsed time. Start time for time-point ["
                  << name << "] was not found\n";
              } else {
                std::cout << "Elapsed time for [" << name << "]: " << time_sec
                  << " sec\n";
              }
            } else {
        ...
        void PrintTotalTime(const char* name){
          PrintTotalTime(std::string(name));
        }
