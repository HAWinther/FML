  ...
  namespace SOLVERS {
    namespace ODESOLVER {
      ...
      struct ODEParamWrapper {
        ...
        public:
          ODEParamWrapper(const ODEFunction& _deriv,
              const ODEFunctionJacobian& _jacobian)
            : deriv(_deriv)
              , jacobian(_jacobian)
        {
        }
          ...
          ODEFunctionPointerJacobian jacobian)
      {
        ...
        gsl_odeiv2_system ode_system = { ode_equation, jacobian,
          size_t(yinitial.size()), parameters };
        gsl_odeiv2_driver* ode_driver = gsl_odeiv2_driver_alloc_y_new(
            &ode_system, stepper, std::abs(hstart) * sign, abserr, relerr);
        ...
        if (verbose) {
          std::cout << "ODESolver step " << std::setw(5) << 0 << " / "
            << num_x_points - 1 << " x: [" << std::setw(10) << x << "] ";
          std::cout << "y: [";
          for (auto& ycomp : y) {
        ...
        for (int i = 1; i < num_x_points; i++) {
          ...
          if (verbose) {
            std::cout << "ODESolver step " << std::setw(5) << i << " / "
              << num_x_points - 1 << " x: [" << std::setw(10) << xnew << "] ";
          }

          // Integrate from x -> xnew
          const int status = gsl_odeiv2_driver_apply(ode_driver, &x, xnew, y.data());
          if (status != GSL_SUCCESS) {
            std::string errormessage = "[ODESolver::solve] GSL gsl_odeiv2_driver_apply returned non-successful\n";
            throw_error(errormessage);
          }
      ...
      DVector2D ODESolver::get_data() const
      {
        return data;
      }
      ...
      DVector2D ODESolver::get_derivative_data() const
      {
