...
namespace FML {
  namespace MATH {
    ...
    std::optional<double> find_root_newton(
        ...
        double epsilon) 
    {
      ...
      if (y_low * y_high > 0.0) {
        throw std::runtime_error(
            "Error binary_search_for_value. f(xlow) and f(xhigh) "
            "have the same sign!");
      }
        ...
        double epsilon)
    {
      ...
      if (y_low * y_high >= 0.0) {
        ...
        for (int i = 0; i < nsearch; i++) {
          ...
          if (i == nsearch - 1) {
            throw std::runtime_error(
                "Error binary_search_for_value. Could not find a good interval to "
                "start search");
          }
        }
      }
    ...
    double j_ell_gsl(const int ell, const double arg){
      // For x=0 we have 0/0
      if (arg == 0.0) return ell == 0 ? 1.0 : 0.0;
      ...
      if(ell < 500 && arg > arg_max_gsl) return 0.0;
    ...
    double j_ell(const int ell, const double arg)
    {
      ...
      if(ell >= 10 && arg < (1.0 - 2.6/sqrt(ell)) * ell) return 0.0;
    ...
    double HyperSphericalBesselWKB(int ell, double nu, double chi, double K){
      if(chi == 0.0) return ell == 0 ? 1.0 : 0.0; 
      ...
      if(sign < 0.0){
        if( K == 0.0 ){
          ...
          S = ( alpha * log( (sqrtw2m1 + sqrtw2pa2) / sqrt(1+a2) ) 
              + atan( sqrtw2pa2 / sqrtw2m1 ) / alpha - M_PI/2.);
        } else {
