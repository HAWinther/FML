    ...
    namespace GADGET {
      ...
      class GadgetWriter{
        ...
        public:
          ...
          template<class T>
            void write_gadget_single(std::string filename, std::vector<T> &part, size_t TotNumPart, int NumberOfFilesToWrite, 
                double aexp, double Boxsize, double OmegaM, double OmegaLambda, double HubbleParam, double pos_norm);
      ...
      template<class T>
        void GadgetReader::read_gadget_single(std::string filename, std::vector<T> &part, bool verbose){
          ...
          if(bytes_in_file <= 256){
            std::string errormessage = "[GadgetReader::read_gadget_single] The file contains less than just a header!\n";
            throw_error(errormessage);
          }

          // Read header
          read_header(fp);
          if(verbose) print_header_info(header);
          ...
          if(bytes_per_particle != bytes_per_particle_expected){
            std::string errormessage = "[GadgetReader::read_gadget_single] BytesPerParticle = " + std::to_string(bytes_per_particle) 
              + " != ExpectedBytes =" + std::to_string(bytes_per_particle_expected) + ". Change the ID size in GadgetUtils?\n";
            throw_error(errormessage);
          }
          ...
          for(auto &field : fields_in_file){
            ...
            if(field == "POS"){
              ...
              if(verbose) std::cout << "Reading POS bytes = " << bytes << " BytesPerParticle: " << sizeof(float) * NDIM << "\n";
              buffer.resize(bytes);
              ...
              if(pos_exists)
                for(int i = 0; i < NumPart; i++){
                  ...
                  for(int idim = 0; idim < NDIM; idim++){
                    ...
                    if(endian_swap) pos[idim] = swap_endian(pos[idim]);
                  }
                }
            }
            ...
            if(field == "VEL"){
              bytes = sizeof(float) * NDIM * NumPart;
              if(verbose) std::cout << "Reading VEL bytes = " << bytes << " BytesPerParticle: " << sizeof(float) * NDIM << "\n";
              buffer.resize(bytes);
              read_section(fp, buffer);
              ...
              if(vel_exists)
                for(int i = 0; i < NumPart; i++){
                  ...
                  for(int idim = 0; idim < NDIM; idim++){
                    ...
                    if(endian_swap) vel[idim] = swap_endian(vel[idim]);
            ...
            if(field == "ID"){
              ...
              if(verbose) std::cout << "Reading ID bytes = " << bytes << " BytesPerParticle: " << sizeof(gadget_particle_id_type) << "\n";
              buffer.resize(bytes);
              read_section(fp, buffer);
              for(int i = 0; i < NumPart; i++){
                if(endian_swap) {
      ...
      template<class T>
        void GadgetWriter::write_gadget_single(std::string filename, std::vector<T> &part, size_t TotNumPart, int NumberOfFilesToWrite, 
            double aexp, double Boxsize, double OmegaM, double OmegaLambda, double HubbleParam, double pos_norm){
          ...
          // Write header
          write_header(fp, 
              NumPart, TotNumPart, NumberOfFilesToWrite, 
              aexp, Boxsize, OmegaM, OmegaLambda, HubbleParam);
