  ...
  namespace FILEUTILS {
    ...
    class LuaFileParser {
      ...
      public:
        ...
        double      read_double(std::string name, double      default_value){ return read_double(name, default_value, false); };
        std::string read_string(std::string name, std::string default_value){ return read_string(name, default_value, false); };
        ...
        // Read arrays of numbers or strings
        template <class T>
          std::vector<T> read_number_array(std::string name, std::vector<T> default_value, bool required = true);
        std::vector<std::string> read_string_array(std::string name, std::vector<std::string> default_value, bool required = true);
    };
    ...
    LuaFileParser::LuaFileParser(std::string filename){
      open(filename);
    }

    LuaFileParser::~LuaFileParser(){
      close();
    }
    ...
    void LuaFileParser::open(std::string filename){
      ...
      if(luaL_loadfile(L, filename.c_str()) || lua_pcall(L, 0, 0, 0)) {
        std::string errormessage = "[LuaFileParser::open] Cannot open parameterfile " + std::string(lua_tostring(L, -1)) + "\n";
    ...
    void LuaFileParser::parameter_not_found(std::string name, bool found, bool required) const{
      if(!found && required) {
        std::string errormessage =  "[LuaFileParser::read] Required parameter " + name + " not found in the parameter file\n";
        throw_error(errormessage);
      }
    }
    ...
    std::vector<std::string> LuaFileParser::read_string_array(std::string name, std::vector<std::string> default_value, bool required){
