  ...
  namespace HESSIAN {
      ...
      void ComputeHessianWithFT(
        ...
        auto ComputeSecondDerivative = [&](FFTWGrid<N> & grid, int i1, int i2){
          ...
          for(auto & fourier_index : grid.get_fourier_range()){
            ...
            if(hessian_of_potential_of_f) factor *= -1.0/kmag2;
            value *= factor;
    ...
    template<int N>
      void SymmetricTensorEigensystem(
        ...
        auto SolveEigensystem = [&](gsl_matrix *_matrix, gsl_vector *_eval, gsl_matrix *_evec, gsl_eigen_symmv_workspace *_workspace){
        ...
        auto SolveEigenvalues = [&](gsl_matrix *_matrix, gsl_vector *_eval, gsl_eigen_symm_workspace *_workspace){
          // Compute eigenvalues and eigenvectors
