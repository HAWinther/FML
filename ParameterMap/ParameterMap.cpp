...
namespace FML {
  namespace UTILS {
    ...
    void ParameterMap::throw_error(std::string errormessage) const{
    ...
    std::map<std::string, ParameterTypes>& ParameterMap::get_map()
    {
      return parameters;
    }
    ...
    template <typename T>
      T ParameterMap::get(std::string name) const
      {
        ...
        try {
          ...
          std::string errormessage = "[ParameterMap::get] Required parameter [" + name + "] was not found in the parameter map\n";
    ...
    bool ParameterMap::contains(std::string name) const
    {
