      ...
      public:
        ...
        void init(std::string type){
          ...
          if(type == "User units"){
            ...
            std::string errormessage = "[ConstantsAndUnits::init] Unknown units: " 
              + type + " Choose between: SI, Planck, ParticlePhysics, Cosmology\n";
        ...
        double length_to_SI(double x){
          return x / m;
        }
        double length_to_user_units(double x){
          return x * m;
        }
        double time_to_SI(double x){
          return x / s;
        }
        double time_to_user_units(double x){
          return x * s;
        }
        double mass_to_SI(double x){
          return x / kg;
        }
        double mass_to_user_units(double x){
          return x * kg;
        }
        double temperature_to_SI(double x){
          return x / K;
        }
        double temperature_to_user_units(double x){
          return x * K;
        }
        double charge_to_SI(double x){
          return x / Co;
        }
        double charge_to_user_units(double x){
          return x * Co;
        }
        double energy_to_SI(double x){
          return x / J;
        }
        double energy_to_user_units(double x){
          return x * J;
        }
        double density_to_SI(double x){
          return x / density;
        }
        double density_to_user_units(double x){
          return x * density;
        }
        double velocity_to_SI(double x){
          return x / velocity;
        }
        double velocity_to_user_units(double x){
          return x * velocity;
        }
        ...
        void info(){
          ...
          std::cout << "============================================\n";
          std::cout << "\n";
          std::cout << "1 Length unit:    " << length_to_SI(1.0)            << " meters = " << length_to_SI(1.0)*(m/Mpc) << " Mpc\n";
          std::cout << "1 Time unit:      " << time_to_SI(1.0)              << " seconds\n";
          std::cout << "1 Mass unit:      " << mass_to_SI(1.0)              << " kg = "     << mass_to_SI(1.0)*(kg/Msun) << " solar masses\n";
          std::cout << "1 Temp unit:      " << temperature_to_SI(1.0)       << " Kelvin\n";
          std::cout << "1 Energy unit:    " << energy_to_SI(1.0)            << " Joules = " << energy_to_SI(1.0) * (J/eV) << " eV\n";
