...
namespace FML {
  namespace INTERPOLATION {
    namespace SPLINE {
      ...
      GSLSpline::GSLSpline(std::string name)
        : GSLSpline()
      {
        this->name = name;
      }
      ...
      GSLSpline& GSLSpline::operator=(const GSLSpline& rhs)
      {
        if (this != &rhs) {
          ...
          if (rhs.spline != nullptr) {
            create(rhs.spline->x, rhs.spline->y, rhs.size_x, rhs.name,
          ...
          const gsl_interp_type* interpoltype)
      {
        ...
        for(int i = 1; i < nx; i++){
          if( (x[i] - x[i-1])*sign_one <= 0 ){
            std::string errormessage = "[GSLSpline::create] x-array for spline " + name + " is not monotone\n";
...
#pragma omp parallel
      ...
      {
        if (x.size() != y.size()) {
          std::string errormessage = "[GSLSpline::create] x and y array must have the same number of elements for spline " + name + "\n";
          throw_error(errormessage);
        }
      ...
      double GSLSpline::deriv_x(double x) const
      {
        return eval_deriv(x, 1);
      }
      double GSLSpline::deriv_xx(double x) const
      {
        return eval_deriv(x, 2);
      }
      std::pair<double, double> GSLSpline::get_xrange() const
      {
        return { xmin, xmax };
      }
      std::string GSLSpline::get_name() const
      {
        return name;
      }
      void GSLSpline::set_out_of_bounds_warning(bool v)
      {
      ...
      GSLSpline2D::GSLSpline2D(std::string name)
        : GSLSpline2D()
      {
        this->name = name;
      }
      ...
      GSLSpline2D& GSLSpline2D::operator=(const GSLSpline2D& rhs)
      {
        if (this != &rhs) {
          // We just create the spline from scratch instead of copying all the data
          if (rhs.spline != nullptr) {
            create(rhs.spline->xarr, rhs.spline->yarr, rhs.spline->zarr, rhs.size_x,
                rhs.size_y, rhs.name, rhs.interpoltype_used);
          } else {
            free();
          }
        }
      ...
      double GSLSpline2D::operator()(double x, double y) const
      {
        return eval(x, y);
      }

      GSLSpline2D::operator bool() const
      {
        return (spline != nullptr);
      }
...
#pragma omp parallel
          ...
          const gsl_interp2d_type* interpoltype)
      {
        if (x.size() * y.size() != z.size()) {
          std::string errormessage = "[GSLSpline2D::create] z array for spline " + name + " have wrong number of elements nx*ny != nz\n";
          throw_error(errormessage);
        }
        create(x.data(), y.data(), z.data(), x.size(), y.size(), splinename,
            interpoltype);
      }
          ...
          const gsl_interp2d_type* interpoltype)
      {
        ...
        if (nz_x * nz_y != nxy) {
          std::string errormessage = "[GSLSpline2D::create] z array for spline " + name + " have wrong dimensions\n";
          throw_error(errormessage);
        }
        ...
        create(x.data(), y.data(), f.data(), x.size(), y.size(), splinename,
            interpoltype);
      }
      ...
      double GSLSpline2D::eval_deriv(double x,
          double y,
          int derivx,
          int derivy) const
      {
        ...
        if (n < 0 || n >= int(derivfunc.size())) {
          std::string errormessage = "[GSLSpline2D::eval_deriv] (" 
            + std::to_string(derivx) + "," + std::to_string(derivy) + ") "
      ...
      double GSLSpline2D::deriv_x(double x, double y) const
      {
        return eval_deriv(x, y, 1, 0);
      }
      double GSLSpline2D::deriv_xx(double x, double y) const
      {
        return eval_deriv(x, y, 2, 0);
      }
      double GSLSpline2D::deriv_y(double x, double y) const
      {
        return eval_deriv(x, y, 0, 1);
      }
      double GSLSpline2D::deriv_yy(double x, double y) const
      {
        return eval_deriv(x, y, 0, 2);
      }
      double GSLSpline2D::deriv_xy(double x, double y) const
      {
        return eval_deriv(x, y, 1, 1);
      }
      std::pair<double, double> GSLSpline2D::get_xrange() const
      {
        return { xmin, xmax };
      }
      std::pair<double, double> GSLSpline2D::get_yrange() const
      {
        return { ymin, ymax };
      }
      std::string GSLSpline2D::get_name() const
      {
        return name;
      }
      void GSLSpline2D::set_out_of_bounds_warning(bool v)
      {
        out_of_bounds_warning = v;
      }
    }
